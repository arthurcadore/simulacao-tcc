...
class Preamble:
    ...
    def plot_preamble(SI, SQ, save_path=None):
        ...
        # Canal I
        axs[0].step(x, SI_up, where='post', label=r"Canal I $(S_I)$", color='blue', linewidth=3)
        ...
        # Canal Q
        axs[1].step(x, SQ_up, where='post', label=r"Canal Q $(S_Q)$", color='red', linewidth=3)
...
if __name__ == "__main__":
    ...
    I_channel = preamble.i_channel()
    Q_channel = preamble.q_channel()
    
    print("Bits:", preamble.preamble_bits)
    print("Preamble I Channel:", I_channel)
    print("Preamble Q Channel:", Q_channel)

    output_path = os.path.join("out", "example_preamble.pdf")
    Preamble.plot_preamble(I_channel, Q_channel, save_path=output_path)